...
cowplot::plot_grid(resplot, qqplot)
```
It seems like this made our data a bit more suited towards a linear model. I don't mind transforming the data in this way because I am not trying to use this model for any interpretations - just making predictions. We will choose a suitable value of lambda with cross validation.

```{r}
# find the value of lambda which results in the lowest cv score
# running this a few times it seems lambda of about 5 is good
lambda = seq(0, 20, by = 0.5)
cv <- transform_cv(rates, lambda, kfolds = 10)
lambda.min = lambda[which(cv == min(cv))]
```

```{r}
pred = data.frame(date = rates$date, predictions = exp(model$fitted.values) - lambda)
ggplot() +
  geom_point(data = rates, mapping = aes(x = date, y = `rate+0`)) +
  geom_line(data = pred, mapping = aes(x = date, y = predictions), colour = "purple")
```

We can see that the model does a pretty nice job! The issue is that we cannot predict all that far into the future, (in fact only 6 months).
